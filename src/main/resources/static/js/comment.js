...
function commentReady(lookNum){
    var lookNum=lookNum;
        ...
        $(window).scroll(function(){   //스크롤 감지 이벤트
            ...
            //윈도우 높이에 스크롤값을 계속더해서 문서 전체 길이에서 170 px 앞에 스크롤이 왔을때 데이터 불러옴
            if((windowHeight + scroll) >= documentHeight-170){
                if(scrollTime==true){
                    commentList(lookNum);
                }
            }
...
function commentList(lookNum) {
    ...
    $.ajax({
        ...
        success:function (result) {
            ...
            setTimeout(function(){scrollTime = true;},500);//스크롤이벤트 0.5초뒤실행 중복방지위해
        },
